...
def get_soft_target_model_updates(target, source, tau):
    ...
    """
    W_t = target.get_weights()
    W = source.get_weights()
    W_t = [(1 - tau) * w_t + tau * w for w_t, w in zip(W_t, W)]
    target.set_weights(W_t)
    W_t = [tf.constant(item) for item in W_t]
    return W_t
...
def get_hard_target_model_updates(target, source):
    ...
    W_t = [tf.constant(item) for item in target.get_weights()]
    return W_t