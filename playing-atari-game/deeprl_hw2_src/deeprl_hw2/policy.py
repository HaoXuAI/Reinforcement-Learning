...
class Policy:
    ...
    def select_action(self, **kwargs):
        ...
        if kwargs.has_key('URP'):
            return UniformRandomPolicy(kwargs['num_actions']).select_action()
        elif kwargs.has_key('GP'):
            return GreedyPolicy().select_action(q_values=kwargs['q_values'])
        elif kwargs.has_key('GEP'):
            return GreedyEpsilonPolicy(kwargs['epsilon']).select_action(q_values=kwargs['q_values'])
...
class GreedyEpsilonPolicy(Policy):
    ...
    def select_action(self, q_values, **kwargs):
        ...
        nA = len(q_values)
        #print "na+", nA
        policy = np.ones(nA, dtype=float) * self.epsilon / nA
...
class LinearDecayGreedyEpsilonPolicy(Policy):
    ...
    def select_action(self, q_values, **kwargs):
        ...
        Any:
          Selected action.
        """
        total_t = kwargs['total_t']
        if kwargs['is_training']:
            self.epsilon = self.epsilons[total_t]
        action_choose = GreedyEpsilonPolicy(self.epsilon)

        action = action_choose.select_action(q_values)
        return action

    def reset(self):
        """Start the decay over at the start value."""
        self.epsilons = np.linspace(start=self.start_value, stop=self.end_value, num=self.num_steps)
